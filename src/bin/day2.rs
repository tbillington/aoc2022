...
impl Score for Round<Hand, Hand> {
    fn score(&self) -> u32 {
        ...
        right.vs(&left).score() + right.score()
    }
}

impl Score for Round<Hand, Outcome> {
    fn score(&self) -> u32 {
        let Round(left, right) = self;

        right.score() + Hand::from((left, right)).score()
    }
}
...
fn main() {
    ...
    println!("part 1: {}", score::<Round<Hand, Hand>>(input));
    println!("part 2: {}", score::<Round<Hand, Outcome>>(input));
}
